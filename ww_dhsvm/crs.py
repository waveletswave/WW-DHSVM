...
def toString(crs: CRS) -> str:
    """A short, human-readable label for a CRS.

    Prefers ``AUTHORITY:CODE`` (e.g. ``EPSG:32618``) with the CRS name
    appended, falling back to the name alone and then to the WKT's first
    line.  Added in WW-DHSVM for concise logging of the model grid,
    where the full WKT would swamp the log.
    """
    if crs is None:
        return 'None'
    try:
    ...
    if code and name and code != name:
        return f'{code} ({name})'
    return code or name or str(crs).splitlines()[0]
