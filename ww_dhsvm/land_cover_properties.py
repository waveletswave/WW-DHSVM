...
import ww_dhsvm.plot
from ww_dhsvm.crs import CRS

from ww_dhsvm.sources.manager_nlcd import colors as nlcd_colors
from ww_dhsvm.sources.manager_modis_appeears import colors as modis_colors
...
def computeTimeSeries(lai: xarray.DataArray,
    ...
    lai timeseries : pandas datafame
       ...
       type.
    """
    lai_crs = ww_dhsvm.crs.from_xarray(lai)
    ...
    df = pd.DataFrame({ww_dhsvm.sources.manager_modis_appeears.colors[int(ilc)][0] + ' LAI [-]' :
...
def applyCrosswalk(crosswalk: Dict[int, List[Tuple[int, float]]],
    ...
    """
    modis_names = ww_dhsvm.sources.manager_modis_appeears.colors

    def _modisName(modis_id):
        return f'{modis_names[modis_id][0]} LAI [-]'

    nlcd_names = ww_dhsvm.sources.manager_nlcd.colors
...
def removeNullLAI(nlcd_lai: pd.DataFrame,
    ...
    if names is None:
        names = ww_dhsvm.sources.manager_nlcd.colors
...
def plotLAI(df: pd.DataFrame,
    ...
    info : type[ww_dhsvm.sources.manager_nlcd.ManagerNLCD] | \
        type[ww_dhsvm.sources.manager_modis_appeears.ManagerMODISAppEEARS] | \
        None = None
    if indices == 'NLCD':
        info = ww_dhsvm.sources.manager_nlcd.ManagerNLCD
    elif indices == 'MODIS':
        info = ww_dhsvm.sources.manager_modis_appeears.ManagerMODISAppEEARS
    else:
...
nlcd_default_properties = pd.read_csv(importlib.resources.files("ww_dhsvm") / "data" / "nlcd_land_cover_properties.csv").set_index('indices')
...
mannings_n_by_stream_order = pd.read_csv(importlib.resources.files("ww_dhsvm") / "data" / "manning_n_by_stream_order.csv").set_index('order')
