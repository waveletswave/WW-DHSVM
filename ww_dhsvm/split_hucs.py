...
import ww_dhsvm.utils
import ww_dhsvm.crs
import ww_dhsvm.plot
import ww_dhsvm.colors
...
class SplitHUCs:
    ...
    def __init__(self,
        ...
        for i in range(len(shapes)):
            for j in range(len(shapes)):
                ...
                if ww_dhsvm.utils.isEmpty(inter):
                    pass
    ...
    @property
    def crs(self) -> ww_dhsvm.crs.CRS:
        return self.df.crs

    def to_crs(self, crs : ww_dhsvm.crs.CRS):
        self.df.to_crs(crs)
    ...
    def plot(self, *args, **kwargs):
        ...
        return ww_dhsvm.plot.linestringsWithCoords(self.df.set_geometry(self.df.boundary), *args, **kwargs)
    ...
    def plotAsLinestrings(self, *args, **kwargs):
        ...
        return ww_dhsvm.plot.linestringsWithCoords(df, *args, **kwargs)
    ...
    def explore(self,
        ...
        if (not 'color' in style_kwds or style_kwds['color'] is None) \
           and not (column in self.df and self.df[column].dtype == 'string'):
            kwargs.setdefault('cmap', matplotlib.colors.ListedColormap(ww_dhsvm.colors.xkcd_muted))
            try:
                ...
                if hole.area < (abs_tol**2) or hole.area < rel_tol * part.area:
                    ...
                    try:
                        i, poly = next(
                            ...
                            if ww_dhsvm.utils.isNonPointIntersection(poly, hole))
...
def partition(list_of_shapes : Sequence[shapely.geometry.Polygon],
    ...
    for i in range(len(list_of_shapes)):
        ...
        for j in range(i + 1, len(list_of_shapes)):
            ...
            try:
                if ww_dhsvm.utils.isVolumetricIntersection(s1, s2):
                    s2 = s2.difference(s1)
...
def intersectAndSplit(list_of_shapes : Sequence[shapely.geometry.Polygon]) -> \
    ...
    for i, s1 in enumerate(list_of_shapes):
        for j, s2 in enumerate(list_of_shapes):
            if i != j and ww_dhsvm.utils.isNonPointIntersection(s1, s2):
    ...
    for i, u in enumerate(uniques):
        if not ww_dhsvm.utils.isEmpty(u):
            uniques_r[i] = u
