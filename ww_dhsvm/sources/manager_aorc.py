...
class ManagerAORC(manager_dataset.ManagerDataset):
    ...
    **Specific Humidity, Pressure, Downward Radiation, Wind**
    ...
    def __init__(self):
        ...
        # AORC v1.1 is extended annually; the bucket now holds 2025.  Track
        # the current year rather than a hard-coded end so a request for the
        # most recent complete year is not rejected out of hand.
        import datetime as _dt
        _last = _dt.date.today().year - (0 if _dt.date.today().month > 6 else 1)
        native_end = cftime.datetime(max(_last, 2024), 12, 31, calendar='standard')
    ...
    def _download(self,
                  ...
                  force: bool = False,
                  geometry_bounds: Optional[tuple] = None,
                  spatial_decimation: int = 1) -> str:
        ...
        if (not os.path.exists(filename)) or force:
            ...
            logging.info(f'Spatial subset size: {ds_subset.nbytes/1e9:.3f} GB '
                         f'({ds_subset.sizes.get("latitude", 0)} x '
                         f'{ds_subset.sizes.get("longitude", 0)} cells)')

            # AORC is 1 km, so a 13,000 km^2 basin holds ~13,000 cells and a
            # year of eight hourly variables is ~25 GB -- while DHSVM, which
            # interpolates from a station list, gains nothing past a few dozen
            # stations.  Decimating keeps *actual AORC cells* (never averages),
            # so every retained station stays physically self-consistent.
            if spatial_decimation > 1:
                ds_subset = ds_subset.isel(
                    latitude=slice(None, None, spatial_decimation),
                    longitude=slice(None, None, spatial_decimation))
                logging.info(
                    f'Decimating by {spatial_decimation} in each direction -> '
                    f'{ds_subset.sizes.get("latitude", 0)} x '
                    f'{ds_subset.sizes.get("longitude", 0)} cells, '
                    f'{ds_subset.nbytes/1e9:.3f} GB')
