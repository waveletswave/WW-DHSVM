...
class ManagerDataset(manager.Manager):
    ...
    def _preprocessParameters(self,
        ...
        polygon = ww_dhsvm.warp.shply(polygon, geometry_crs, self.native_crs_in)
    ...
    def _postprocessDataset(self,
        ...
        if dataset.rio.crs is None:
            ...
            assert ww_dhsvm.crs.isEqual(ww_dhsvm.crs.from_rasterio(dataset.rio.crs),
                                           self.native_crs_out)
        ...
        for dim in [d for d in dataset.dims
            ...
            if isinstance(dataset.indexes[dim], pd.DatetimeIndex):
                new_time = ww_dhsvm.data.convertTimesToCFTime(dataset[dim].values)
                if self.native_calendar == 'noleap':
                    new_time = ww_dhsvm.data.convertTimesToCFTimeNoleap(new_time)
        ...
        if request.out_crs is not None:
            ...
            dataset = ww_dhsvm.warp.dataset(dataset, request.out_crs, request.resampling)
