...
def get_basic_argparse(docstring):
    ...
    parser.add_argument('--logfile', type=str, help='Write logging to file instead of stdout')
    return parser
...
def projection(parser):
    ...
    parser.add_argument('--projection',
                        type=valid_epsg,
                        default=ww_dhsvm.crs.default_crs(),
...
def valid_hucstr(hucstr):
    try:
        huc_valid = ww_dhsvm.sources.utils.huc_str(hucstr)
...
def huc_arg(parser):
    ...
    parser.add_argument('HUC', type=valid_hucstr, help='HUC code, for example "060102080101"')
    ...
    def valid_level(x):
        if x % 2 != 0:
            raise argparse.ArgumentTypeError(
                "In parsing huc_level: '{}' must be a multiple of 2 to be a valid HUC level.".
                format(x))
        return x

    parser.add_argument('--level', type=int, default=0, help='Level of HUCs to include.')
    return parser
...
def default_simplify_options():
    ...
    args = Struct(simplify=10., prune_reach_size=2, cut_intersections=False, verbosity=1)
    return args
...
def inshape_args(parser):
    ...
    parser.add_argument('input_file', type=shapefile, help='filename including shape to be meshed')
    parser.add_argument('--shape-index',
                        type=int,
                        default=-1,
...
def outmesh_args(parser):
    ...
    parser.add_argument('output_file', type=str, help='VTK Filename for mesh output.')
    parser.add_argument('-p', '--plot', action='store_true', help='Save mesh image to file.')
...
def huc_hint_options(parser):
    ...
    parser.add_argument(
        ...
        type=valid_hucstr,
        help=
        "Hint for searching for a HUC, i.e. '06' for Tennessee River.  Note currently required to avoid downloading all HUCs!",
        required=True)
...
def center_options(parser):
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=
        "Center the output mesh for cleaner simulation but harder provenance.  Note centroid is written to the output mesh's readme file"
    )
...
def huc_source_options(parser):
    ...
    parser.add_argument('--source-huc',
                        type=str,
                        default=ww_dhsvm.sources.default_huc_source,
                        choices=set(ww_dhsvm.sources.huc_sources.keys()),
                        help='Hydrologic unit shapefile dataset.  (default = "{}")'.format(
                            ww_dhsvm.sources.default_huc_source))
...
def dem_source_options(parser):
    ...
    parser.add_argument('--source-dem',
                        type=str,
                        default=ww_dhsvm.sources.default_dem_source,
                        choices=set(ww_dhsvm.sources.dem_sources.keys()),
                        help='Digital Elevation Model dataset.  (default = "{}")'.format(
                            ww_dhsvm.sources.default_dem_source))
...
def hydro_source_options(parser):
    ...
    parser.add_argument('--source-hydro',
                        type=str,
                        default=ww_dhsvm.sources.default_hydrography_source,
                        choices=set(ww_dhsvm.sources.hydrography_sources.keys()),
                        help='Hydrography dataset.  (default = "{}"'.format(
                            ww_dhsvm.sources.default_hydrography_source))
